...
# Use conda-forge's Fortran symbol convention, including when BLAS++ was
# cross-compiled without running its mangling probe.
...
if [[ "${CONDA_BUILD_CROSS_COMPILATION:-}" == "1" ]]; then
    ...
    export CMAKE_ARGS="${CMAKE_ARGS} -Dlapack_libraries_cached=${PREFIX}/lib/liblapack${SHLIB_EXT}"
    export CMAKE_ARGS="${CMAKE_ARGS} -Dlapack_config_cache=${PREFIX}/lib/liblapack${SHLIB_EXT}"
